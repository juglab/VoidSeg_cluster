...
from scripts.utils.utils import precision
...
for exp in exp_names:
    ...
    for run in conf[exp]:
        ...
        for frac in fraction_keys:
            ...
            for gt_k, result_k in zip(gt_image_keys, result_image_keys):
                precision_score += precision(gt_images[gt_k], result_images[result_k])

            precision_score /= float(len(result_image_keys))
            if frac in run_precision_scores.keys():
                run_precision_scores[frac].append(precision_score)
            else:
                run_precision_scores[frac] = [precision_score]
    ...
    max = {}
    n_run = {}
    for r in run_precision_scores.keys():
        ar = np.array(run_precision_scores[r])
        n_run[r] = ar.shape[0]
    ...
    keys = [x for x in run_precision_scores.keys()]
    keys.sort()
