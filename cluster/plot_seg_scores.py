...
for exp in exp_paths:
    ...
    for k in subdirs_keys:
        seg_score = 0
        result_files = glob(join(subdirs[k], '*.tif'))
        result_images = {int(basename(x)[:-4]) : imread(x) for x in result_files}
        ...
        for gt_img_key, result_img_key in zip(gt_images_keys, result_image_keys):
            score = seg(gt_images[gt_img_key], result_images[result_img_key])
